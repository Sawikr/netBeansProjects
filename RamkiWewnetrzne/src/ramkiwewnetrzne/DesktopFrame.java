...
public class DesktopFrame extends JFrame
{
   ...
   public DesktopFrame()
   {
      ...
      final JCheckBoxMenuItem dragOutlineItem = new JCheckBoxMenuItem("Drag Outline");// przeciągnij kontur
      ...
      iframe.addVetoableChangeListener(new VetoableChangeListener()
         {
            public void vetoableChange(PropertyChangeEvent event) throws PropertyVetoException
            {
               ...
               if (name.equals("closed") && value.equals(true))// nazwa closed ma tu znaczenie (to właściwość metody setClosed(true) - strona 496 J.T.Z9)
                                                               // ale zadziała jakakolwiek nazwa
               {
   ...
   public void tileWindows()
   {
      ...
      int rows = (int) Math.sqrt(frameCount);
        System.out.println("Liczba rows: " + rows);
      int cols = frameCount / rows;
        System.out.println("Liczba cols: " + cols);
      int extra = frameCount % rows;
        System.out.println("Liczba extra: " + extra);
   ...
   public void selectNextWindow()
   {
      ...
      for (int i = 0; i < frames.length; i++)
      {
         if (frames[i].isSelected())
         {
            ...
            System.out.println("Liczba next z selectNextWindow(): " + next);
            while (next != i)
            {
               ...
               next = (next + 1) % frames.length;
                System.out.println("Next z pętli while: " + next);
            }
         }
      }
   }
